...
import { Activity, Deadline, KeyEvent, Trip, TripFilters, TripStats } from "~/types/trips";
...
export function useTrips(filters?: TripFilters) {
  ...
  const stats = useMemo<TripStats>(() => {
    ...
    const totalSpent = trips.reduce((sum: number, trip: Trip) => sum + (trip.spentSoFar || 0), 0);
    ...
    const relocatingTrips = trips.filter((trip: Trip) => trip.status === "relocating").length;
  ...
  const updateTrip = useCallback(async (idOrSlug: string, updates: Partial<Trip>) => {
    ...
    setTrips((prev: Trip[]) =>
      prev.map((trip: Trip) => (trip.id === updatedTrip.id ? updatedTrip : trip))
    );
    return updatedTrip;
  }, []);
  ...
  const updateKeyEvent = useCallback(
    async (tripId: string, eventId: string, updates: Partial<KeyEvent>) => {
  ...
  const updateDeadline = useCallback(
    async (tripId: string, deadlineId: string, updates: Partial<Deadline>) => {
